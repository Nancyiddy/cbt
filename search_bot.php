			...
			$query_search = mysqli_query($mysqli, "SELECT * FROM exams WHERE name LIKE '%$search_id%' OR score LIKE '%$search_id%' OR passcode LIKE '%$search_id%'");
			$result_search = mysqli_num_rows($query_search);
			if($result_search >=1){
			...
			<tbody>
			<?php
			while($show_search = mysqli_fetch_array($query_search)){
			?>
			<tr>
				<td><?php echo $show_search[1]; ?></a></td>
				<td><?php echo $show_search[2]; ?></td>
				<td><?php if($show_search[2] >=50){ echo 'Passed'; }else{ echo 'Failed'; }; ?></td>
